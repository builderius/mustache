...
class FilesystemCache extends AbstractCache
{
    ...
    {
        $fileName = $this->getCacheFilename($key);
        $this->log(Logger::DEBUG, 'Writing to template cache: "{fileName}"', array('fileName' => $fileName));
    ...
    private function buildDirectoryForFilename($fileName)
    {
        ...
        if (!\is_dir($dirName)) {
            $this->log(Logger::INFO, 'Creating Mustache template cache directory: "{dirName}"', array('dirName' => $dirName));
    ...
    private function writeFile($fileName, $value)
    {
        ...
        $this->log(Logger::DEBUG, 'Caching compiled template to "{fileName}"', array('fileName' => $fileName));
        ...
        if (\false !== @\file_put_contents($tempFile, $value)) {
            ...
            // @codeCoverageIgnoreStart
            $this->log(Logger::ERROR, 'Unable to rename Mustache temp cache file: "{tempName}" -> "{fileName}"', array('tempName' => $tempFile, 'fileName' => $fileName));
            // @codeCoverageIgnoreEnd
        }
        // @codeCoverageIgnoreStart
